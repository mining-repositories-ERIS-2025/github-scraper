...
def gitstar_ranking_generator(pages: int):
...
# Example usage: Print the first N pages
repository_rank = 0
one_year_ago = datetime.now() - timedelta(days=365)
commit_count = 0
commit_limit = 10000

for repo in gitstar_ranking_generator(1):
    repository_rank += 1
    if repo[1] != "Python":
    ...
    for commit in Repository(repo[0], since=one_year_ago).traverse_commits():
        ...
        try:
            ...
            if any(keyword in commit.msg.lower() for keyword in KEYWORDS):
                if commit.lines < 20:
                    if any(file.filename.endswith('.py') for file in commit.modified_files):
                        ...
                        with open("commits.jsonl", "a", encoding="utf-8") as f:
                            f.write(json.dumps(commit_object) + "\n")
            ...
            with open("all_commits.jsonl", "a", encoding="utf-8") as f:
                f.write(json.dumps(commit_object) + "\n")
